...
LTE_fdd_enb_cnfg_db::LTE_fdd_enb_cnfg_db()
{
   /* LTE_fdd_enb_interface *interface = LTE_fdd_enb_interface::get_instance();
    ...
    FILE                  *cnfg_file = NULL;
    int64                  set_param_one;
    double                 set_param_two;
    double                 set_param_three;
    char                   str[LTE_FDD_ENB_MAX_LINE_SIZE];
    int64                  set_param_four;
    int64                  set_param_five;
    int64                  set_param_six;
    int64                  set_param_seven;
    int64                  set_param_eight;
    int64                  set_param_night;
    cnfg_file = fopen("/tmp/lzh.cnfg_db", "r");
    int num = 1;
    if(NULL != cnfg_file)
    {
	...
	switch(num)
{
	case 1:	set_param_two=atoi(line_str.c_str());
	num++;
	break;
	case 2: set_param_one=atoi(line_str.c_str())-1;
	num++;
	break;
	case 3: set_param_three=atoi(line_str.c_str());
	num++;
	break;
	case 4:	set_param_four=atoi(line_str.c_str());
	num++;
	break;
	case 5:	set_param_five=atoi(line_str.c_str());
	num++;
	break;
	case 6:	set_param_six=atoi(line_str.c_str());
	num++;
	break;
	case 7:	set_param_seven=atoi(line_str.c_str());
	num++;
	break;
	case 8:	set_param_eight=(int)strtol(line_str.c_str(),NULL,16);
	num++;
	break;
	case 9:	set_param_night=(int)strtol(line_str.c_str(),NULL,16);
	num++;
	break;
	default:
	break;
    ...
    var_map_double[LTE_FDD_ENB_PARAM_BANDWIDTH]                = 10.0000;
    var_map_int64[LTE_FDD_ENB_PARAM_FREQ_BAND]                 = 1;
    var_map_int64[LTE_FDD_ENB_PARAM_DL_EARFCN]                 = liblte_interface_first_dl_earfcn[0];
    //var_map_int64[LTE_FDD_ENB_PARAM_DL_EARFCN]                 = set_param_three;
    ...
    var_map_int64[LTE_FDD_ENB_PARAM_N_ID_CELL]                 = 1;
...
void LTE_fdd_enb_cnfg_db::read_cnfg_file(void)
{
	...
	set_param(LTE_FDD_ENB_PARAM_BANDWIDTH,num);
        fclose(cnfg_file);
    }
}
