...
void LTE_fdd_enb_mme::handle_nas_msg(LTE_FDD_ENB_MME_NAS_MSG_READY_MSG_STRUCT *nas_msg)
{
    ...
    if(LTE_FDD_ENB_ERROR_NONE == nas_msg->rb->get_next_mme_nas_msg(&msg))
    {
        ...
        case LTE_FDD_ENB_MME_TAU_REQUEST:
                                           ...
                                           LTE_fdd_enb_rb         **rb)
{
    ...
    interface->send_debug_msg(LTE_FDD_ENB_DEBUG_TYPE_INFO,
                              ...
                              __FILE__,
                              __LINE__,
                              "Received Attach Request for RNTI=%u",
                              (*user)->get_c_rnti());
    ...
    if(LIBLTE_MME_EPS_MOBILE_ID_TYPE_GUTI == attach_req.eps_mobile_id.type_of_id)
    {
        if(LTE_FDD_ENB_ERROR_NONE == user_mgr->find_user(&attach_req.eps_mobile_id.guti, &act_user))
        {
            ...
            interface->send_debug_msg(LTE_FDD_ENB_DEBUG_TYPE_INFO,
                                      ...
                                      __FILE__,
                                      __LINE__,
                                      "IMSI=%015llu is associated with RNTI=%u",
                                      (*user)->get_id()->imsi,
                                      (*user)->get_c_rnti());
        ...
        interface->send_debug_msg(LTE_FDD_ENB_DEBUG_TYPE_INFO,
                                  ...
                                  __FILE__,
                                  __LINE__,
                                  "IMEI=%015llu is associated with RNTI=%u",
                                  imei_num,
                                  (*user)->get_c_rnti());
                                        ...
                                        LTE_fdd_enb_rb          **rb)
{
    ...
    interface->send_debug_msg(LTE_FDD_ENB_DEBUG_TYPE_INFO,
                              ...
                              __FILE__,
                              __LINE__,
                              "Received TAU Request for m-tmsi=0x%X",
			      0xFF01706C	
			);
    else
   interface->send_debug_msg(LTE_FDD_ENB_DEBUG_TYPE_INFO,
                                                     ...
                                                     LTE_fdd_enb_rb         *rb)
{
    ...
    interface->send_debug_msg(LTE_FDD_ENB_DEBUG_TYPE_INFO,
                              ...
                              __FILE__,
                              __LINE__,
                              "Received PDN Connectivity Request for RNTI=%u",
                              user->get_c_rnti());
                                ...
                                LTE_fdd_enb_rb   *rb)
{
    ...
    interface->send_debug_msg(LTE_FDD_ENB_DEBUG_TYPE_INFO,
                              ...
                              __FILE__,
                              __LINE__,
                              "send TAU Reject for m_tmsi=0x%X, EMM Cause=9",
                              0xFF01706C);
    else
   interface->send_debug_msg(LTE_FDD_ENB_DEBUG_TYPE_INFO,
                              LTE_FDD_ENB_DEBUG_LEVEL_MME,
                              __FILE__,
                              __LINE__,
                              "Received TAU Reject for m-tmsi=0x%X, EMM Cause=12",
			      0xD2342732	
			);
    // Queue the NAS message for RRC
    (rb)->queue_rrc_nas_msg(&msg);
                                         ...
                                         LTE_fdd_enb_rb   *rb)
{
    ...
    interface->send_debug_msg(LTE_FDD_ENB_DEBUG_TYPE_INFO,
                              ...
                              __LINE__,
                              &msg,
                              "Sending Attach Reject for IMSI=%015llu, EMM Cause=2",
                              imsi_num);
    super_test++;
